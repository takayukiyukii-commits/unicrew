...
> 🚨 git の手順自体は 素の git で同じ手順を再現したスクリプト 相当で毒味済み（2026-09-03）。ここで見るのは「アプリがその手順を正しい順番で呼んでいるか」。
...
> 🚨 git の手順自体は 素の git で同じ手順を再現したスクリプト で毒味済み（2026-09-03：M/D/R/A の検出・ターン基準で絞れる・利用者の index/HEAD 不変・worktree でも同手順）。ここで見るのは「アプリが正しい cwd と基準で呼んでいるか」。
...
> 🚨 git の手順自体は 素の git で同じ手順を再現したスクリプト で毒味済み（2026-09-03：追加・変更・削除の3種が戻る／HEAD・index・ブランチ不変／.gitignore 対象は触らない／別スレッドの記録は拒否／50本で打ち切り）。
